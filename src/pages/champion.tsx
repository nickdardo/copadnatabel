...
export default function ChampionPage() {
  ...
  // Prize pool based on confirmed payments
  ...
  async function handleSave() {
    ...
    const isEdit = !!champion // editing existing pick
    ...
    await supabase.from('champion_picks').upsert({
      ...
      locked: newEditCount >= MAX_CHAMP_EDITS,
    ...
    <Layout title="Palpite de Campeão">
      <div className="max-w-md mx-auto px-4 py-5 space-y-4">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#E6F4FA] flex items-center justify-center flex-shrink-0 text-[#0099CC]">
            <IcoTrophy />
          </div>
          <div>
        ...
        {paidCount > 0 && (
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
              ...
              <span className="text-[12px] font-semibold text-[#0099CC]">{paidCount} pagamentos · {formatBRL(prizePool)}</span>
            </div>
            <div className="grid grid-cols-3 divide-x divide-gray-100">
              {[
                ...
                <div key={pos} className={`${bg} flex flex-col items-center py-4 px-2`}>
                  <Icon size={36} />
                  <p className="text-[11px] font-semibold text-gray-500 mt-2">{pos} colocado · {pct}%</p>
            ...
            <div className="px-4 py-2.5 bg-gray-50 border-t border-gray-100">
              <p className="text-[11px] text-gray-400 text-center flex items-center justify-center gap-1.5">
                <IcoInfo />
                Premiação atualiza conforme pagamentos confirmados pelo admin
              </p>
            </div>
          </div>
        )}
        ...
        {paidCount === 0 && (
          <div className="bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 flex items-center gap-2.5">
            ...
            <p className="text-[12px] text-gray-500">
              O valor da premiação será exibido aqui conforme os pagamentos forem confirmados.
            </p>
          </div>
        )}
          ...
          {POSITIONS.map(({ label, pts, border, ptColor }) => (
            <div key={label} className={`rounded-xl border p-3 text-center ${border}`}>
              <div className="flex justify-center mb-1.5">
                {label === 'Campeão'      ? <MedalGold   size={34} /> :
                 label === 'Vice-campeão' ? <MedalSilver size={34} /> :
                                            <MedalBronze size={34} />}
              </div>
            ...
            <div className="flex-1">
              <p className={`text-[12px] font-semibold ${locked ? 'text-red-700' : editsLeft === 1 ? 'text-amber-700' : 'text-blue-700'}`}>
                {locked
                  ...
                  : `${editsLeft} alteração${editsLeft === 1 ? '' : 'ões'} restante${editsLeft === 1 ? '' : 's'}`}
              </p>
            ...
            <div key={label} className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 pt-3.5 pb-2">
                <div className="flex items-center gap-2">
                  {label === 'Campeão'      ? <MedalGold   size={22} /> :
                   label === 'Vice-campeão' ? <MedalSilver size={22} /> :
                                              <MedalBronze size={22} />}
              ...
              {state ? (
                <div className="mx-4 mb-2 flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2.5 border border-gray-100">
                  ...
                  {!locked && (
                    <button onClick={() => set('')} className="ml-auto text-[11px] text-gray-400 hover:text-red-500 transition-colors">
                      Trocar
                    </button>
                  )}
                </div>
          ...
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-red-500 mt-0.5">
                {editCount >= MAX_CHAMP_EDITS
                  ? `Você atingiu o limite de ${MAX_CHAMP_EDITS} alterações.`
        ...
        {!locked && (
          <button
            ...
            className="w-full py-4 rounded-xl font-bold text-[15px] text-white flex items-center justify-center gap-2 transition-all active:scale-[.98] bg-[#0099CC] hover:bg-[#007aa8] disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
          >
            {saved ? (
              <><IcoCheck /> Salvo! Abrindo palpites...</>
            ) : saving ? (
              <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : isFirstSave ? (
              <>Salvar palpite de campeão <IcoArrow /></>
            ) : (
              <>Atualizar palpite ({editsLeft} de {MAX_CHAMP_EDITS} alt. restantes) <IcoArrow /></>
            )}
          </button>
        )}
