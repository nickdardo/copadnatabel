...
import { supabase, Score, Player, FACTOR_COLOR, getAvatarUrl } from '@/lib/supabase'
import Layout from '@/components/Layout'
import { IconTrophy, IconGold, IconSilver, IconBronze } from '@/components/Icons'

type RankEntry = Score & { player: Player }

const AVATAR_COLORS = ['#0099CC','#378ADD','#D85A30','#7F77DD','#D4537E','#BA7517','#639922','#533AB7']
...
export default function RankingPage() {
  ...
  async function fetchRanking() {
    const { data } = await supabase.from('scores').select('*, players(*)').order('total_pts', { ascending: false })
    if (data) {
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
          ...
          <div className="bg-white border border-[#0099CC]/20 rounded-2xl p-4 mb-4"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <MedalIcon pos={myPos} />
                {/* Avatar */}
                {getAvatarUrl(myEntry.player.avatar_url) ? (
                  <img src={getAvatarUrl(myEntry.player.avatar_url)!} alt=""
                    className="w-10 h-10 rounded-full object-cover border-2 border-white shadow" />
                ) : (
                  <div className="w-10 h-10 rounded-full bg-[#E6F4FA] flex items-center justify-center text-[13px] font-bold text-[#0099CC] border-2 border-white shadow">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-gray-400">Minha posição</div>
                </div>
              </div>
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              ...
              {myEntry.champion_pts > 0 && <span className="text-[11px] px-2 py-0.5 rounded-md font-semibold bg-amber-100 text-amber-800">+{myEntry.champion_pts} camp.</span>}
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          {ranking.map((entry, i) => {
            ...
            const photoUrl = getAvatarUrl(entry.player.avatar_url)
            const displayName = entry.player.nickname || entry.player.username
              ...
              <div key={entry.player_id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {photoUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0 shadow-sm"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-900 text-[14px] truncate flex items-center gap-1.5">
                      {displayName}
                      {isMe && <span className="text-[11px] text-[#0099CC] font-medium">(você)</span>}
                      {!entry.player.payment_ok && (
                        <span className="text-[9px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md">
                          Pag. pendente
                        </span>
                      )}
                    </div>
