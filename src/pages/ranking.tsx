...
const AVATAR_COLORS = ['#0099CC','#1565C0','#7B1FA2','#C62828','#2E7D32','#F57F17','#00838F','#4527A0']

function initials(name: string) {
  return (name || '?').split(' ').filter(Boolean).map((w:string)=>w[0]).slice(0,2).join('').toUpperCase()
}

function resolveAvatar(player: Player): string | null {
...
const FACTOR_LABELS: Record<string, string> = {
  f10_count:'F10', f7_count:'F7', f5_count:'F5', f2_count:'F2', f0_count:'F0'
}
...
export default function RankingPage() {
  ...
  async function fetchRanking() {
    const { data: scoresData } = await supabase
      .from('scores').select('*, players(*)').order('total_pts', { ascending: false })
    if (!scoresData) { setFetching(false); return }

    // Fetch champion picks
    const playerIds = scoresData.map((d:any) => d.player_id)
    const { data: champData } = await supabase
      .from('champion_picks').select('*').in('player_id', playerIds)
      ...
      <div className="max-w-lg mx-auto">
            ...
            <div className="absolute inset-0 opacity-10">
              {Array.from({length:8}).map((_,i) => (
                <div key={i} className="absolute rounded-full border border-white/30"
                  style={{ width: (i+1)*80, height:(i+1)*80, top:'50%', left:'50%',
            ...
            <div className="relative flex items-center gap-4 px-5 py-5">
              ...
              <div className="relative flex-shrink-0">
                {resolveAvatar(myEntry.player) ? (
                  ...
                  <div className="w-16 h-16 rounded-full border-3 border-white flex items-center justify-center text-xl font-bold text-[#0099CC] bg-white shadow-lg"
                    style={{border:'3px solid white'}}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-bold text-[18px] leading-tight truncate">
                  {(myEntry.player.nickname || myEntry.player.username).toUpperCase()}
                </p>
                <p className="text-white/70 text-[11px] mt-0.5 font-medium">
                  F10: {myEntry.f10_count} · F7: {myEntry.f7_count} · F5: {myEntry.f5_count} · F2: {myEntry.f2_count} · F0: {myEntry.f0_count}
                  {myEntry.champion_pts > 0 && ` · Bônus: +${myEntry.champion_pts}`}
                </p>
                {/* Champion picks flags */}
                {myEntry.champ && (
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <span className="text-white/60 text-[10px]">Palpites:</span>
                    <span className="text-base">{getFlag(myEntry.champ.pick_champion)}</span>
                    <span className="text-base">{getFlag(myEntry.champ.pick_runner)}</span>
                    <span className="text-base">{getFlag(myEntry.champ.pick_third)}</span>
                  </div>
                )}
              </div>
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-white/60 text-[11px]">pontos</p>
              </div>
            </div>

            {/* Payment badge */}
            {!myEntry.player.payment_ok && (
              <div className="bg-amber-400/20 border-t border-amber-400/30 px-5 py-2 flex items-center gap-2">
        ...
        <div>
          {ranking.map((entry, i) => {
            ...
            const photoUrl = resolveAvatar(entry.player)
            const name    = entry.player.nickname || entry.player.username
            const isOpen  = expanded === entry.player_id

            // Row background for top 3
            const rowBg =
              i === 0 ? 'bg-[#00b4d8]/8 border-b border-[#00b4d8]/20' :
              i === 1 ? 'bg-[#0077b6]/6 border-b border-[#0077b6]/15' :
              i === 2 ? 'bg-[#48cae4]/5 border-b border-[#48cae4]/10' :
              ...
              <div key={entry.player_id}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3.5 transition-colors text-left
                    ${rowBg} ${isMe ? 'bg-[#0099CC]/5' : ''} hover:bg-gray-50/50`}>
                  ...
                  <div className="w-8 flex items-center justify-center flex-shrink-0">
                    {i === 0 ? (
                      ...
                      <span className="text-[13px] font-bold text-gray-400">{i+1}º</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`font-bold text-[14px] truncate ${isTop3 ? 'text-gray-900' : 'text-gray-700'}`}>
                        {name}
                      </span>
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5 font-medium">
                      F10: {entry.f10_count} · F7: {entry.f7_count} · F5: {entry.f5_count} · F2: {entry.f2_count} · F0: {entry.f0_count}
                    </p>
                  </div>
                ...
                {isOpen && (
                  <div className="bg-gray-50 border-b border-gray-100 px-5 py-4">
                    <div className="flex items-start justify-between gap-4">
                      {/* Factor breakdown */}
                      <div>
                        <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-2">Pontuação</p>
                        <div className="flex gap-2 flex-wrap">
                          {[
                            { k:'f10_count', label:'F10', color:'bg-green-100 text-green-800',  pts:10 },
                            { k:'f7_count',  label:'F7',  color:'bg-blue-100 text-blue-800',   pts:7  },
                            { k:'f5_count',  label:'F5',  color:'bg-amber-100 text-amber-800', pts:5  },
                            { k:'f2_count',  label:'F2',  color:'bg-pink-100 text-pink-800',   pts:2  },
                            { k:'f0_count',  label:'F0',  color:'bg-gray-100 text-gray-600',   pts:0  },
                          ].map(({ k, label, color, pts }) => (
                            <div key={k} className={`px-2.5 py-1.5 rounded-xl text-center ${color}`}>
                              <p className="text-[10px] font-semibold">{label}</p>
                              <p className="text-[14px] font-bold">{(entry as any)[k]}</p>
                              <p className="text-[9px] opacity-70">{(entry as any)[k] * pts}pts</p>
                            </div>
                          ))}
                      ...
                      {entry.champ && (
                        <div className="text-right">
                          <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-2">Palpites</p>
